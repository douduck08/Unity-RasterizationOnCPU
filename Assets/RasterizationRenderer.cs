...
public class PixelBuffer {
    ...
    // Varyings
    public List<Vector4> positions = new List<Vector4> (); // positions in NDC
}
...
public class RasterizationRenderer : MonoBehaviour {
    ...
    void Draw () {
        ...
        Clear (colorBuffer, depthBuffer, backgroundColor);
    ...
    void Clear (Color[] colorBuffer, float[] depthBuffer, Color color, float depth = 1f) {
    ...
    void ClippingAndRasterization (VertexBuffer vertexBuffer, PixelBuffer pixelBuffer) {
        ...
        for (int i = 0; i < size; i += 3) {
            var v0 = new Vector2 (vertexBuffer.positions[i].x / vertexBuffer.positions[i].w, vertexBuffer.positions[i].y / vertexBuffer.positions[i].w);
            var v1 = new Vector2 (vertexBuffer.positions[i + 1].x / vertexBuffer.positions[i + 1].w, vertexBuffer.positions[i + 1].y / vertexBuffer.positions[i + 1].w);
            var v2 = new Vector2 (vertexBuffer.positions[i + 2].x / vertexBuffer.positions[i + 2].w, vertexBuffer.positions[i + 2].y / vertexBuffer.positions[i + 2].w);
            ...
            for (int x = xMin; x < xMax; x++) {
                for (int y = yMin; y < yMax; y++) {
                    ...
                    if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
                        ...
                        var position = vertexBuffer.positions[i] * (w0 / area) + vertexBuffer.positions[i + 1] * (w1 / area) + vertexBuffer.positions[i + 2] * (w2 / area);
                        position /= position.w;
